...
export function PreviewForm({ schema }: PreviewFormProps) {
	...
	function renderField(field: Field, path: string = "") {
		...
		switch (field.type) {
			...
			case "boolean":
				return (
					<div key={fullPath} className="mb-4 flex items-center gap-2">
			...
			case "object":
				return (
					<fieldset key={fullPath} className="border p-3 rounded mb-4">
		...
		<div className="mb-4">
			<label className="block mb-1 text-sm">
				...
				<button
					...
					onClick={() =>
						fa.append(
							itemType === "number" ? 0 : itemType === "boolean" ? false : "",
						)
					}
				>
					Add
